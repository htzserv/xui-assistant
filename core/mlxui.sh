...
install_panels() {
    ...
    for ((i = 1; i <= panel_count; i++)); do
        ...
        if docker ps -a --format "{{.Names}}" | grep -q "^tx-ui-$i$"; then
            echo "A container with the name 'tx-ui-$i' already exists. Removing it..."
            docker stop "tx-ui-$i" > /dev/null 2>&1
            docker rm "tx-ui-$i" > /dev/null 2>&1
        fi
        ...
        if [ $? -eq 0 ]; then
            ...
            echo "Sub Port: $sub_port"
            echo "Inbound Ports: $inbound_ports"
            panel_ports["tx-ui-$i"]="Panel Port: $panel_port, Sub Port: $sub_port, Inbound Ports: $inbound_ports"
        else
...
update_panels() {
    ...
    panels=($(docker ps --format "{{.Names}}" | grep "tx-ui-" || true))
    ...
    while true; do
        ...
        case $choice in
        1)
            read -p "Enter the name of the panel to update (e.g., tx-ui-1): " panel_name
...
remove_panels() {
    ...
    while true; do
        ...
        case $choice in
        1)
            read -p "Enter the name of the panel to remove (e.g., tx-ui-1): " panel_name
            if docker ps --format "{{.Names}}" | grep -q "^$panel_name$"; then
